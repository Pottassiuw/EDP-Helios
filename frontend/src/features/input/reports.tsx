...
function extrairDesvioMeses(desvio: unknown): number | null {
  const match = String(desvio ?? '').match(/\((-?\d+(?:\.\d+)?)m\)/);
...
export function Reports({
  ...
  const registrosSLA = React.useMemo(() => {
    return registrosBase.map((nota): SLADados => ({
      nota,
      statusSLA: String(nota.Status_SLA ?? 'Dados Insuficientes'),
      desvio: extrairDesvioMeses(nota.Desvio_SLA),
      textoDesvio: String(nota.Desvio_SLA ?? 'Dados Insuficientes'),
    }));
  }, [registrosBase]);
