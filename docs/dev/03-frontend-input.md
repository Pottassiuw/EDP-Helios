...
| `frontend/src/features/input/logs.tsx` | Sub-aba "Logs": três sub-abas (Alterações nas Notas, Arquivos, Linha do Tempo), cada uma consumindo um endpoint próprio via `useQuery`. "Arquivos" (`GET /logs/arquivos`) lista substituições de base e também exportações — por isso o rótulo deixou de ser "Bases de Apoio". "Alterações nas Notas" pagina no servidor (100 por página, `placeholderData: keepPreviousData` para a página anterior não piscar enquanto a próxima carrega); busca por nota e linha do tempo têm debounce de 300ms; contadores por tipo e lista de usuários vêm do próprio endpoint. |
